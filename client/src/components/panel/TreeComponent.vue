...
<div style="height:100%;">
  <div style="padding:1em;">
    <v-text-field dense hide-details autocomplete="off" v-model="search_text">
      <v-icon slot="prepend" color="#757575">
...
export default {
  ...
  methods: {
    ...
    removeChildren(item) {
      var parent_node = this.search(this.mrx_store.app.node_list, item.parent);
      var rm_idx = parent_node.children.indexOf(item);
      parent_node.children.splice(rm_idx, 1);
    },
    onLoadProcess(item) {
      if(item.type == 'Process') {
        this.mrx_store.app.selected_proc = item;
      } else {
  ...
  computed : {
    search_items() {
      if(this.search_text) {
        return this.searchNode(this.mrx_store.app.node_list, this.search_text)
      } else {
        return this.mrx_store.app.node_list
      }
    }
  },
  data () {
  ...
  mounted() {
    ...
    console.log('mounted')
    this.onLoadProcess(this.mrx_store.app.node_list[0])
  },
  updated() {
