...
<div style="width:100%;height:100%;">
  ...
  <v-tabs-items v-model="tab" style="width:100%;height:calc(100% - 40px);overflow:none;">
    <v-tab-item v-for="item in tabs" :key="item.name" style="height: 100%; width: 100%">
      ...
      <div @dragover="dragover" @drop="drop" style="width:100%;height:calc(100% - 36px);z-index:1;" ref="flow_container"></div>
      
      <div class="side-btn">
        <label style="font-size:0.8em">Snap</label>
        <div>
...
export default {
  ...
  methods: {
    ...
    drop: function(e) {
      ...
      if (this.mrx_store.app.selected_proc.id) {
        var node = {
          idx: null,
          parent:this.mrx_store.app.selected_proc.id,
          id: this.$common.utils.uuid(),
          name: "",
        ...
        this.mrx_store.app.selected_proc.children.push(node);
      } else {
        this.$message({
          type: 'warning',
    ...
    render_edge() {
      ...
      this.mrx_store.app.edge_list.forEach(function (d) {
    ...
    onShortCut(e) {
      if (e.keyCode == 46) {
        this.cy.elements(':selected').forEach(function (d) {
          if(d.data().type == 'Edge') {
            var rm_edge = this.mrx_store.app.edge_list.findIndex(function(t) {
              return t.id == d.id()
            })
            
            if(this.mrx_store.app.selected_type == 'Edge' && rm_edge == this.mrx_store.app.selected_idx) this.mrx_store.app.selected_idx = -1;
            this.mrx_store.app.edge_list.splice(rm_edge, 1);

            this.render_edge();
          } else {
            var rm_idx = this.mrx_store.app.selected_proc.children.findIndex(function (t) {
              return t.id == d.id()
            });
            if(this.mrx_store.app.selected_type == 'Node' && rm_idx == this.mrx_store.app.selected_idx) this.mrx_store.app.selected_idx = -1;
            this.mrx_store.app.selected_proc.children.splice(rm_idx, 1)

            this.mrx_store.app.edge_list = this.mrx_store.app.edge_list.filter(function(t, i) {
  ...
  mounted() {
    ...
    this.cy = cytoscape({
      ...
      zoom: 0.722719885372964,
      container: this.$refs.flow_container, // container to render in
      layout: {
        name: "preset"
      },
      elements: {
      ...
      commands: [{
          ...
          select: function (org, evt) {
            if (this.mrx_store.app.selected_proc.id) {
              ...
              this.mrx_store.app.selected_proc.children.push(ele);
            } else {
              this.$message({
                type: 'warning',
                message: 'TREE ITEM을 더블 클릭해주세요.'
    ...
    this.handles.init(this.cy, function (edge) {
      this.mrx_store.app.edge_list.push(edge.json().data);
    }.bind(this));
    ...
    this.cy.on(
      ...
      function (evt) {
        ...
        if(node.data().type == 'Edge') {
          var selected_idx = this.mrx_store.app.edge_list.findIndex(function(d) {
            return d.id == node.id();
          })
          if(selected_idx > -1) {
            this.mrx_store.app.selected_type = 'Edge';
            this.mrx_store.app.selected_idx = selected_idx;
          }
        } else {
          var selected_idx = this.mrx_store.app.selected_proc.children.findIndex(function (d) {
            return d.id == node.id()
          })
          if(selected_idx > -1) {
            this.mrx_store.app.selected_type = 'Node';
            this.mrx_store.app.selected_idx = selected_idx;
          }
        }
      }.bind(this)
    ...
    this.cy.on('position', function(evt) {
      ...
      this.mrx_store.app.dragging = true;
    }.bind(this))
    this.cy.on('dragfree', function(evt) {
      if (evt.target === this.cy) return;

      var node = evt.target;
      var selected_node = this.mrx_store.app.selected_proc.children.find(function (d) {
        return d.id == node.id()
      })
      
      this.mrx_store.app.dragging = false;
      if(selected_node) selected_node['position'] = evt.target.position();
    }.bind(this))

    this.render(this.mrx_store.app.selected_proc);
    this.render_edge();
  ...
  destroyed() {
    ...
    this.handles.close();
    // this.$common.store.setProperty('app.node_list', this.cy.nodes().map(function (d) {
    //   return d.json()
    // }));
  }
};
