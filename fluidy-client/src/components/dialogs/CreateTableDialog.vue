<template>
  <v-dialog
    ...
    :persistent="true"
  >
    <v-dialog v-model="showOptionsDialog" max-width="450px" :persistent="false">
      <v-card tile outlined>
            ...
            <v-row>
              <v-col>
                <v-data-table
                  ...
                  :items="schema_list[editedIndex].options.items"
                  calculate-widths
        ...
        <v-system-bar window height="36" color="rgb(42,42,42)">
          ...
          <span>Set up column options.</span>
          <v-spacer></v-spacer>
          <v-icon color="success" @click="saveOptions">mdi-check-bold</v-icon>
          <v-icon color="red" @click="closeOptions">mdi-close-thick</v-icon>
        </v-system-bar>
      </v-card>
    </v-dialog>
    <v-card tile outlined>
      ...
      <v-system-bar window height="36" color="rgb(42,42,42)">
        <v-icon small>mdi-information-outline</v-icon>
        <span>Set up table name and schema.</span>
        <v-spacer></v-spacer>
...
export default {
  created() {
    mainEventBus.$on("OPEN_CREATE_TABLE_DIALOG", () => {
  ...
  methods: {
    ...
    async createContent() {
       ...
       * 순서
       * 1. api 요청
       * 2. 성공시 tree data 재 로드, currentContents 추가
       *
       */
