      ...
      <gl-row :height="50">
        <gl-stack :closable="false">
          <gl-component title="Form">
            ...
            <div
              style="height: calc(100% - 36px); width: 100%; overflow: auto;display:flex;justify-content:center;align-items;"
            >
              ...
              <div class="side-btn">
                ...
                <div style="display: flex">
                  <v-tooltip left v-if="!expand">
                    ...
                    <span
                      >드래그 앤 드랍으로 배치하며, 더블 클릭시 다른 노드를 선택
                      가능합니다.</span
                    >
                  </v-tooltip>
                  <div
...
export default {
  props: ["content_guid", "content_options"],
  components: {},
  data() {
    return {
      grid_snap: true,
    ...
    grid_snap: {
      ...
      handler(val) {
        ...
        this.$message({
          type: "info",
          message:
            "Grid Snap 기능이 " + (val ? "활성화" : "비활성화") + "되었습니다.",
        });
      },
    },
  ...
  methods: {
    ...
    goToParent() {
      var parent_node = this.search(
        this.store.flow.node_list,
        this.store.flow.selected_proc.parent
      );
      if (parent_node) {
    ...
    render_edge() {
      ...
      this.store.flow.edge_list.forEach(
        function (d) {
          if (
            this.cy.nodes("#" + d.source).length > 0 &&
            this.cy.nodes("#" + d.target).length > 0
          ) {
            this.cy.add({
              data: d,
              classes: "taxi",
    ...
    onShortCut(e) {
      if (e.keyCode == 46) {
        this.cy.elements(":selected").forEach(
          function (d) {
            if (d.data().type == "Edge") {
              ...
              if (
                ...
                rm_edge == this.store.flow.selected_idx
              )
                this.store.flow.selected_idx = -1;
              this.store.flow.edge_list.splice(rm_edge, 1);

              this.render_edge();
            } else {
              var rm_idx = this.store.flow.selected_proc.children.findIndex(
                function (t) {
                  return t.id == d.id();
                }
              );
              if (
                ...
                rm_idx == this.store.flow.selected_idx
              )
                this.store.flow.selected_idx = -1;
              this.store.flow.selected_proc.children.splice(rm_idx, 1);

              this.store.flow.edge_list = this.store.flow.edge_list.filter(
                function (t, i) {
  ...
  mounted() {
    ...
    this.cy.cxtmenu({
        menuRadius: function(ele){ return 20; }, // the outer radius (node center to the end of the menu) in pixels. It is added to the rendered size of the node. Can either be a number or function as in the example.
    ...
    this.cy.on(
      ...
      function (evt) {
        ...
        if (node.data().type == "Edge") {
          ...
          var selected_idx = this.store.flow.selected_proc.children.findIndex(
            function (d) {
              return d.id == node.id();
            }
          );
    ...
    this.cy.on(
      ...
      function (evt) {
        ...
        var node = evt.target;
        var selected_node = this.store.flow.selected_proc.children.find(
          function (d) {
            return d.id == node.id();
          }
        );
